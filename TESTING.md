...
against — and the private-use digit recovery and letterspaced-small-caps
rejoining were both written against a real Cambridge University Press journal
article and checked through the actual app. What that does not cover is real
books at large, which are messier than any one article or a fixture I wrote
myself.

If numbers go missing from a converted PDF, that is the private-use case: check
the message under the text box, which says how many glyphs could not be read.
